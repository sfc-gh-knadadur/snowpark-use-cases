...
```bash
  docker build --platform linux/amd64 -t robustvideomatting .
 ``` 
2. Tag the image 
```bash
docker tag robustvideomatting:latest  <repository_url>/robustvideomatting:latest
...
docker push  <repository_url>/robustvideomatting:latest
...
```sql
create service robustvideomatting
    in compute pool compute_pool_gpu_S
    from @specs
    specification_file='robustvideomatting.yaml'
    external_access_integrations = (ALLOW_ALL_EAI);
```

3. See that the service has started by executing `SHOW SERVICES IN COMPUTE POOL compute_pool_gpu_S` 
  and `SELECT system$get_service_status('robustvideomatting')`.
4. Find the public endpoint for the service by executing `SHOW ENDPOINTS IN SERVICE robustvideomatting`.


## Use Jupyter-Lab
1. Navigate to the endpoint and authenticate with a user that has access.

2. Navigate to the folder /app/robustvideomatting/RobustVideoMatting/RobustVideoMatting-master 

3. Upload the Demo_Notebook.ipynb to the /app/robustvideomatting/RobustVideoMatting/RobustVideoMatting-master
